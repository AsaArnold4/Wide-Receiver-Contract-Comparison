...
def load_espn_ids() -> dict:
    try:
        here = os.path.dirname(os.path.abspath(__file__))
        with open(os.path.join(here, "espn_headshot_ids.json")) as f:
            return json.load(f)
...
def render_tier_card(rep, tier: str, espn_ids: dict):
    ...
    inj_html = f'&nbsp;<span style="font-size:0.7rem;color:#c8924a;">{inj}</span>' if inj else ""
    signing  = rep["signing_label"]
    apy_str  = fmt(rep["inflated_apy"])
    gtd_str  = fmt(rep["inflated_guaranteed"])
    pff_str  = f"{rep['grades_pass_route']:.1f}"
    yprr_str = f"{rep['yprr']:.2f}"
    yds_str  = f"{rep['yards']:.0f}"
    sim_str  = f"{rep['similarity_score']:.3f}"
    apy_cap_str = f"{apy_cap:.2f}%"
    gtd_pct_str = f"{gtd_pct:.0f}%"
...
def render_result_row(row, rank: int, espn_ids: dict):
    ...
    inj_html  = f'<span style="color:#c8924a;font-size:0.72rem;">{inj}</span>' if inj else ""
    signing   = row["signing_label"]
    apy_str   = fmt(row["inflated_apy"])
    gtd_str   = fmt(row["inflated_guaranteed"])
    yprr_str  = f"{row['yprr']:.2f}"
    pff_str   = f"{row['grades_pass_route']:.1f}"
    yds_str   = f"{row['yards']:.0f}"
    adot_str  = f"{row['avg_depth_of_target']:.1f}"
    apy_cap_str = f"{apy_cap:.2f}%"
    gtd_pct_str = f"{gtd_pct:.0f}%"
    pill       = pill_html(tier)
