...
def build_pff_profile(player_name: str, pff_seasons: dict[int, pd.DataFrame],
    ...
    # Store raw weighted counting stats for the agent brief
    feature_dict["_targets"]      = wavg("targets")
    feature_dict["_receptions"]   = wavg("receptions")
    feature_dict["_touchdowns"]   = wavg("touchdowns")
    feature_dict["_routes"]       = w_routes
...
def lookup_player(raw_name: str, eng: dict,
    ...
    if not matches.empty:
        ...
        # Raw counting stats for agent brief
        feature_dict["_targets"]      = row.get("targets", None)
        feature_dict["_receptions"]   = row.get("receptions", None)
        feature_dict["_touchdowns"]   = row.get("touchdowns", None)
        feature_dict["_routes"]       = row.get("routes", None)
...
FALLBACK = (
    "https://static.www.nfl.com/image/private/f_auto,q_auto/league/shield"
)

@st.cache_data(show_spinner=False)
def get_espn_headshot(player_name: str) -> str:
    """
    Dynamically look up a player's correct ESPN ID via the ESPN search API,
    then return the headshot CDN URL. Cached per player name.
    Falls back to NFL shield if the player isn't found.
    """
    ...
    try:
        ...
        url   = (
            ...
            f"?query={query}&limit=5&type=athlete&sport=football&league=nfl"
        )
        req  = urllib.request.Request(url, headers={"User-Agent": "Mozilla/5.0"})
        with urllib.request.urlopen(req, timeout=4) as resp:
            data = json.loads(resp.read())

        # Walk results and find the best name match
        key = normalize_name(player_name)
        for item in data.get("items", []):
            candidate = item.get("displayName", "")
            if normalize_name(candidate) == key:
                uid = item.get("id", "")
                if uid:
                    return f"https://a.espncdn.com/i/headshots/nfl/players/full/{uid}.png"

        # Fallback: just take the first result if only one hit
...
def headshot_url(player_name: str, _unused: dict = None) -> str:
    """Wrapper kept for API compatibility — ignores the old espn_ids dict."""
    return get_espn_headshot(player_name)
...
def render_tier_card(rep, tier: str, espn_ids: dict):
    ...
    html = (
        ...
        '<img src="' + hs + '" class="hs-sm" onerror="this.src=&quot;' + FALLBACK + '&quot;" />' +
        '<div style="flex:1;">' +
...
def render_result_row(row, rank: int, espn_ids: dict):
    ...
    html = (
        ...
        str(rank) + "</div>" +
        '<img src="' + hs + '" class="hs-xs" onerror="this.src=&quot;' + FALLBACK + '&quot;" />' +
...
# ══════════════════════════════════════════════════════════════════════════════
#  AGENT BRIEF  —  Claude-generated player summary
# ══════════════════════════════════════════════════════════════════════════════

def get_first_pff_season(player_name: str, pff_seasons: dict) -> int | None:
    """Return the earliest season year the player appears in PFF data."""
    key = normalize_name(player_name)
    for yr in sorted(pff_seasons.keys()):
        df = pff_seasons[yr]
        if (df["player_key"] == key).any():
            return yr
    return None
...
def generate_agent_brief(
    ...
    """
    Call the Anthropic API to generate a one-paragraph agent brief.
    All inputs are plain Python types so Streamlit can cache the result.
    """
    import json as _json

    # Year entering on new deal
    if first_season:
        years_in_league  = most_recent_season - first_season + 1
        year_entering    = years_in_league + 1
        year_str         = f"entering his {year_entering}{'st' if year_entering==1 else 'nd' if year_entering==2 else 'rd' if year_entering==3 else 'th'} NFL season"
    else:
        year_str = "entering a new contract year"

    prompt = (
        f"Write a single short paragraph (3-4 sentences) summarizing this NFL wide receiver "
        f"for a sports agent preparing for a contract negotiation. Use exactly this structure:\n\n"
        f"1. State the player\'s name, their age (you know who this player is), and that they are {year_str} "
        f"with the {team}.\n"
        f"2. Give their key stats: {int(receptions)} receptions for {int(yards)} yards and "
        f"{int(touchdowns)} touchdowns on {int(targets)} targets, with a PFF route grade of {pff_grade:.1f} "
        f"and {yprr:.2f} yards per route run.\n"
        f"3. State that comparable signings project a market value of "
        f"${low_apy:.1f}M to ${high_apy:.1f}M annually, with {comp_names} as the most similar contracts.\n\n"
        f"Be factual and direct. Do not editorialize or add analysis beyond what is stated. "
    ...
    try:
        ...
        # Graceful fallback — plain template if API fails
        return (
            f"{player_name} is {year_str} with the {team}. "
            f"He recorded {int(receptions)} receptions for {int(yards)} yards and "
            f"{int(touchdowns)} touchdowns on {int(targets)} targets, posting a PFF route grade "
            f"of {pff_grade:.1f} and {yprr:.2f} yards per route run. "
            f"Comparable contracts project a market value of ${low_apy:.1f}M\u2013${high_apy:.1f}M annually, "
            f"with {comp_names} as the most similar signings."
        )

def main():
    eng         = load_engine()
    espn_ids    = {}   # no longer used — headshots fetched dynamically
    ...
    if run_query and feature_dict is not None:
        ...
        # Agent Brief
        st.markdown('<div class="sec-title">Agent Brief</div>',
                    unsafe_allow_html=True)

        with st.spinner("Generating brief…"):
            # Gather inputs for the brief
            _team        = feature_dict.get("_team", "") or ""
            _seasons     = feature_dict.get("_seasons_used") or []
            _most_recent = max(_seasons) if _seasons else max(pff_seasons.keys()) if pff_seasons else 2024
            _first       = get_first_pff_season(player_name or "", pff_seasons)

            # Top 3 realistic comps by name
            _top3 = (
                tiered[tiered["tier"] == "realistic"]["player"]
                .head(3).tolist()
            )
            if not _top3:
                _top3 = tiered["player"].head(3).tolist()
            _comp_names = ", ".join(_top3) if _top3 else "similar players"

            brief_text = generate_agent_brief(
                player_name        = player_name or "",
                team               = _team or "their current team",
                first_season       = _first,
                most_recent_season = _most_recent,
                yards              = clean_fd.get("yards", 0),
                touchdowns         = feature_dict.get("_touchdowns") or 0,
                pff_grade          = clean_fd.get("grades_pass_route", 0),
                yprr               = clean_fd.get("yprr", 0),
                targets            = feature_dict.get("_targets") or 0,
                receptions         = feature_dict.get("_receptions") or 0,
                low_apy            = pcts["p25"] / 1e6,
                high_apy           = pcts["p75"] / 1e6,
                comp_names         = _comp_names,
            )

        st.markdown(
            "<div style=\"background:#10101e;border:1px solid #1c1c32;border-left:3px solid #00e5a0;"
            "border-radius:12px;padding:1.25rem 1.5rem;margin-bottom:1.4rem;"
            "font-size:1rem;line-height:1.7;color:#ddddf0;\">"
            + brief_text +
            "</div>",
            unsafe_allow_html=True,
        )
